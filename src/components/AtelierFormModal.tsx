...
export default function AtelierFormModal({ atelier, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (atelier) {
      setForm({
        titre: atelier.titre, description: atelier.description,
        date: atelier.date, heure: atelier.heure, duree: atelier.duree,
        lieu: atelier.lieu, prix: String(atelier.prix), places_max: String(atelier.places_max),
      })
    ...
    try {
      ...
      if (imageFile) {
        ...
        const { error: uploadError } = await supabase.storage
          .from('images').upload(fileName, imageFile, { upsert: true })
        if (uploadError) throw uploadError
        const { data: urlData } = supabase.storage.from('images').getPublicUrl(fileName)
        image_url = urlData.publicUrl
      }
      const payload = {
        titre: form.titre, description: form.description, date: form.date,
        heure: form.heure, duree: form.duree, lieu: form.lieu,
        prix: parseFloat(form.prix), places_max: parseInt(form.places_max),
        places_restantes: parseInt(form.places_max), image_url,
      }
      if (isEdit && atelier) {
        const { error } = await supabase.from('ateliers')
          .update({ ...payload, places_restantes: atelier.places_restantes }).eq('id', atelier.id)
      ...
      <div className="bg-white rounded-3xl border-4 border-[#1A1040] shadow-pop w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            >
              {imagePreview ? (
                <img src={imagePreview} alt="aperçu" className="w-full h-40 object-cover rounded-xl border-2 border-[#1A1040]" />
              ) : (
          ...
          {field('🏷️ Titre de l\'atelier', 'titre', 'text', 'Ex: Broderie Kawaii')}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {field('👥 Nb places', 'places_max', 'number', '8')}
          </div>
