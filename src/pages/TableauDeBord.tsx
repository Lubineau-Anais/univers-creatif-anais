      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp className="w-4 h-4 text-[#1A1040]" />
            <h2 className="text-sm font-black text-[#1A1040] uppercase tracking-wide">
              Statistiques {anneeSelectionnee}
            </h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-citron-400 rounded-2xl p-5 border-2 border-[#1A1040] text-[#1A1040]"
              ...
              <div className="mt-2 h-1.5 bg-[#1A1040]/20 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#1A1040] rounded-full transition-all duration-700"
          ...
          <div className="px-6 py-4 border-b-2 border-[#1A1040] flex items-center justify-between">
            <div>
              <h3 className="font-black text-[#1A1040] flex items-center gap-2">
                ...
                CA HT mensuel — {anneeSelectionnee}
              </h3>
          ...
          <div className="px-6 py-6">
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center gap-1 group relative">
                    ...
                    <div
                      className={`w-full rounded-t-lg transition-all duration-500 ${
                        estMoisCourant
                          ? 'bg-citron-400 border-2 border-[#1A1040]'
                          : estVide
                ...
                return (
                  <div key={idx} className="flex-1 text-center">
                    <span className={`text-[10px] font-black ${
                      estMoisCourant ? 'text-[#1A1040]' : 'text-gray-400'
                    }`}>
                      {mois}
                    </span>
                    {caParMois[idx] > 0 && (
                      <div className="text-[9px] text-gray-400 font-bold truncate">
                        {caParMois[idx] >= 1000
                          ? `${(caParMois[idx] / 1000).toFixed(1)}k`
                          : caParMois[idx]}€
                      </div>
                    )}
                  </div>
            ...
            <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-gray-500 font-bold">CA encaissé</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 bg-citron-400 border border-[#1A1040] rounded-sm" />
                <span className="text-xs text-gray-500 font-bold">Mois en cours</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 bg-gray-100 border border-gray-200 rounded-sm" />
                <span className="text-xs text-gray-500 font-bold">Aucun CA</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* ── FILTRES ── */}
        <div className="bg-white rounded-2xl border-2 border-[#1A1040] p-4 space-y-4"
          style={{ boxShadow: '3px 3px 0px 0px #1A1040' }}>
          {/* Filtre période */}
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-sm font-black text-[#1A1040] flex items-center gap-1.5">
            ...
            {[
              ...
              <button key={value}
                onClick={() => setFilterPeriode(value as typeof filterPeriode)}
                className={`px-4 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                  filterPeriode === value ? activeStyle : 'bg-white text-[#1A1040] border-[#1A1040] hover:bg-candy'
                }`}>
                {label}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {[
              ...
              <button key={value}
                ...
                className={`px-4 py-1.5 rounded-xl text-xs font-black border-2 transition-all ${
                  filterStatut === value ? style : 'bg-white text-[#1A1040] border-[#1A1040] hover:bg-candy'
                }`}>
                {label}
              </button>
            ))}
        ...
        {ateliersFiltres.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border-4 border-[#1A1040]"
            style={{ boxShadow: '5px 5px 0px 0px #ffb5c8' }}>
            <div className="text-6xl mb-4">
              {filterPeriode === 'avenir' ? '🔮' : filterPeriode === 'passes' ? '📁' : '📭'}
            </div>
            <p className="font-black text-[#1A1040] text-xl">
              {ateliersDeLAnnee.length === 0
                ? `Aucun atelier en ${anneeSelectionnee}.`
          ...
          ateliersFiltres.map(atelier => {
            const reservsFiltrees = atelier.reservations.filter(r =>
              filterStatut === 'tous' || r.statut_paiement === filterStatut
            )
            ...
            const tauxAtelier = atelier.places_max > 0
              ? Math.round((nbInscrits / atelier.places_max) * 100) : 0
            ...
            return (
              <div key={atelier.id}
                className={`bg-white rounded-3xl border-4 overflow-hidden ${isPasse ? 'border-gray-300 opacity-80' : 'border-[#1A1040]'}`}
                style={{ boxShadow: isPasse ? '5px 5px 0px 0px #d1d5db' : '5px 5px 0px 0px #ffb5c8' }}>

                {/* En-tête */}
                <button
                  className={`w-full px-6 py-4 flex items-center gap-4 transition-colors ${isPasse ? 'bg-gray-600 hover:bg-gray-700' : 'bg-[#1A1040] hover:bg-[#2d2060]'}`}
                  ...
                  <div className="flex-1 flex flex-col md:flex-row md:items-center gap-2 text-left">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="flex items-center gap-1 bg-white/10 text-gray-300 px-2.5 py-1 rounded-full text-xs font-bold">
                        ...
                        <span className="capitalize">{fmtDate(atelier.date)}</span>
                      </span>
                      <span className="bg-white/10 text-gray-300 px-2.5 py-1 rounded-full text-xs font-bold">
                        🕐 {atelier.heure} · {atelier.duree}
                      </span>
                      <span className="bg-citron-400/20 text-citron-400 px-2.5 py-1 rounded-full text-xs font-bold">
                        👥 {nbInscrits}/{atelier.places_max} — {tauxAtelier}% rempli
                      </span>
                      {nbPresents > 0 && (
                        <span className="bg-lime-300/30 text-lime-300 px-2.5 py-1 rounded-full text-xs font-bold">
                          ✅ {nbPresents} présent{nbPresents > 1 ? 's' : ''}
                        </span>
                      )}
                      <span className="bg-lime-300/20 text-lime-400 px-2.5 py-1 rounded-full text-xs font-bold">
                        💶 {caAtelier} € HT
                      </span>
                    </div>
                  </div>
                    ...
                    {reservsFiltrees.length === 0 ? (
                      <div className="text-center py-10 text-gray-400">
                        ...
                        <p className="text-sm font-bold">
                          {atelier.reservations.length === 0 ? 'Aucune réservation.' : 'Aucune réservation pour ce filtre.'}
                        </p>
                      </div>
                    ) : (
                      <table className="w-full text-sm">
                            ...
                            return (
                              <tr key={r.id}
                                className={`border-b border-gray-100 transition-colors ${
                                  estPresent ? 'bg-lime-50 hover:bg-lime-100/70'
                                  : idx % 2 === 0 ? 'bg-white hover:bg-rose-50/50'
                                  : 'bg-candy/50 hover:bg-rose-50/50'
                                }`}>
                                {/* Case présence */}
                                <td className="px-4 py-3 text-center">
                                  <label className="flex items-center justify-center cursor-pointer group">
                                    ...
                                    <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${
                                      estPresent ? 'bg-lime-400 border-lime-500' : 'bg-white border-gray-300 group-hover:border-[#1A1040]'
                                    }`}>
                                      {estPresent && (
                                ...
                                <td className="px-4 py-3 text-gray-500 whitespace-nowrap text-xs">
                                  {fmtDateCourt(r.created_at)}<br/>
                                  <span className="text-gray-400">{fmtHeure(r.created_at)}</span>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <select
                                    value={r.statut_paiement}
                                    disabled={updatingId === r.id}
                                    onChange={e => updateStatut(r.id, e.target.value)}
                                    className={`px-3 py-1.5 rounded-xl text-xs font-black border-2 cursor-pointer appearance-none focus:outline-none focus:ring-2 focus:ring-[#1A1040] transition-all ${
                                      r.statut_paiement === 'paye'       ? 'bg-lime-100 text-lime-700 border-lime-400'
                                      : r.statut_paiement === 'en_attente' ? 'bg-red-100 text-red-600 border-red-400'
                                      : 'bg-gray-100 text-gray-500 border-gray-300'
                                    } ${updatingId === r.id ? 'opacity-50 cursor-wait' : ''}`}
                                  >
                                    {STATUT_OPTIONS.map(opt => (
                                      <option key={opt.value} value={opt.value}>
                                        {opt.value === 'paye' ? '✅ Payé' : opt.value === 'en_attente' ? '🔴 En attente' : '❌ Annulé'}
                                      </option>
                                    ))}
                                  </select>
                                </td>
                        ...
                        <tfoot>
                          <tr className={`border-t-2 ${isPasse ? 'bg-gray-600' : 'bg-[#1A1040]'}`}>
                            ...
                            <td colSpan={5} className="px-4 py-3 text-white text-xs font-black">
                              {reservsFiltrees.length} participant{reservsFiltrees.length > 1 ? 's' : ''}
                            </td>
                            <td className="px-4 py-3" />
                            <td className="px-4 py-3">
                            ...
                            <td className="px-4 py-3">
                              <span className="text-lime-400 font-black text-sm">
                                ...
                                <span className="text-citron-400">
                                  {reservsFiltrees.filter(r => r.statut_paiement === 'paye').length * atelier.prix} € HT
                                </span>
                              </span>
                            </td>
                          </tr>
        ...
        {/* ── BOUTON ARCHIVER MANUELLEMENT ── */}
        {anneeSelectionnee < currentYear && !showArchiveBanner && (
          <div className="text-center pt-4">
            <button
              onClick={() => archiverAnnee(anneeSelectionnee)}
              disabled={isArchiving}
